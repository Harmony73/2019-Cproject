 ...
 *注意事项：需要# #include <stdio.h>，double的处理时间略长但更精准。
 */
double TransToDouble(char treat[]);
 ...
 */
void ExchangeMarks(char expression[],int i,Stack* pstack);
/*功能：调整括号后的运算符与逗号的顺序。
 *输入参数：已输出的逆波兰式，逆波兰式脚标。
 *空函数不返回。
 */ 
...
void TypeInAndTransport(char expression[]) {
	...
	char character[1000];
	int i = 0, j = 0;
	ConstructStack(&stack1);
	ConstructStack(&stack2);
	gets(character);
	while (1) {
		if (character[j] == 0)
			break;
		...
		case '^':
			expression[i] = ',';
			ExchangeMarks(expression,i,&stack2);
			i++;
			for (; ComparePriority(character[j], expression, i, &stack1) == 1;)
				i++;
			break;
		case '(':	SaveInStack(&character[j], &stack1);
			break;
		case ')':
			for (; expression[i - 1] != '(';) {
				PopFromStack(expression, i, &stack1);
				i++;
			}
			i--;
			expression[i] = 0;
	...
	i = PopAllDatas(expression, i, &stack1);
	for (j = i - 1;; j--) {
		if (expression[j] >= '0' && expression[j] <= '9')
			break;
		SaveInStack(&expression[j], &stack1);
	}
	j += 2;
	expression[j - 1] = ',';
	i = PopAllDatas(expression, j, &stack1);
	expression[i] = '\0';
}
/*功能：逆波兰式转化过程入栈
 *参数：当前读取字符character，栈的地址
...
/*功能：调整括号后的运算符与逗号的顺序。
 *输入参数：已输出的逆波兰式，逆波兰式脚标,栈的地址 
 *空函数不返回。
 */ 
void ExchangeMarks(char expression[],int i,Stack* pstack){
	for(;expression[i-1]=='+'||expression[i-1]=='-'
		||expression[i-1]=='*'||expression[i-1]=='/'
		||expression[i-1]=='^';i--)
		SaveInStack(&expression[i-1],pstack);
	expression[i]=',';
	PopAllDatas(expression,i+1,pstack);
} 
